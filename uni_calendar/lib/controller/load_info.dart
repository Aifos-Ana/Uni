...
import 'package:uni_calendar/controller/local_storage/image_offline_storage.dart';
...
Future loadRemoteUserInfoToState(Store<AppState> store) async {
  ...
  final Completer<Null> userInfo = Completer(),
      ...
      feupCalendar = Completer(),
      sigarraEvents = Completer();
  ...
  store.dispatch(getFeupCalendarFromFetcher(feupCalendar));
  store.dispatch(getSigarraEventsFromFetcher(sigarraEvents));
...
void loadLocalUserInfoToState(store) async {
  ...
  if (userPersistentInfo.item1 != '' && userPersistentInfo.item2 != '') {
    ...
    store.dispatch(updateStateBasedOnLocalUserFeupCalendar());
    store.dispatch(updateStateBasedOnLocalUserSigarraEvents());
