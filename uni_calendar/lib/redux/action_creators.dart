...
import 'package:uni_calendar/controller/feup_calendar_fetcher/feup_calendar_fetcher_html.dart';
import 'package:uni_calendar/controller/load_info.dart';
import 'package:uni_calendar/controller/load_static/terms_and_conditions.dart';
import 'package:uni_calendar/controller/local_storage/app_bus_stop_database.dart';
import 'package:uni_calendar/controller/local_storage/app_courses_database.dart';
import 'package:uni_calendar/controller/local_storage/app_events_database.dart';
import 'package:uni_calendar/controller/local_storage/app_exams_database.dart';
import 'package:uni_calendar/controller/local_storage/app_last_user_info_update_database.dart';
import 'package:uni_calendar/controller/local_storage/app_lectures_database.dart';
import 'package:uni_calendar/controller/local_storage/app_refresh_times_database.dart';
import 'package:uni_calendar/controller/local_storage/app_shared_preferences.dart';
import 'package:uni_calendar/controller/local_storage/app_sigarra_events_database.dart';
import 'package:uni_calendar/controller/local_storage/app_user_database.dart';
import 'package:uni_calendar/controller/local_storage/app_restaurant_database.dart';
import 'package:uni_calendar/controller/networking/network_router.dart'
    show NetworkRouter;
import 'package:uni_calendar/controller/parsers/parser_courses.dart';
import 'package:uni_calendar/controller/parsers/parser_exams.dart';
import 'package:uni_calendar/controller/parsers/parser_fees.dart';
import 'package:uni_calendar/controller/parsers/parser_print_balance.dart';
import 'package:uni_calendar/controller/restaurant_fetcher/restaurant_fetcher_html.dart';
import 'package:uni_calendar/controller/schedule_fetcher/schedule_fetcher.dart';
import 'package:uni_calendar/controller/schedule_fetcher/schedule_fetcher_api.dart';
import 'package:uni_calendar/controller/schedule_fetcher/schedule_fetcher_html.dart';
import 'package:uni_calendar/controller/sigarra_events_fetcher/sigarra_events_fetcher_html.dart';
import 'package:uni_calendar/model/app_state.dart';
import 'package:uni_calendar/model/entities/calendar_item.dart';
import 'package:uni_calendar/model/entities/course.dart';
import 'package:uni_calendar/model/entities/course_unit.dart';
import 'package:uni_calendar/model/entities/exam.dart';
import 'package:uni_calendar/model/entities/lecture.dart';
import 'package:uni_calendar/model/entities/profile.dart';
import 'package:uni_calendar/model/entities/restaurant.dart';
import 'package:uni_calendar/model/entities/session.dart';
import 'package:uni_calendar/model/entities/trip.dart';
...
ThunkAction<AppState> updateStateBasedOnLocalUserFeupCalendar() {
  return (Store<AppState> store) async {
    final AppEventsDatabase db = AppEventsDatabase();
    final List<CalendarItem> items = await db.events();
    store.dispatch(SetFeupCalendar(items));
  };
}

ThunkAction<AppState> getFeupCalendarFromFetcher(Completer<Null> action){
  return (Store<AppState> store) async{
    try{
      final List<CalendarItem> events =
      await FeupCalendarFetcherHtml().getFeupCalendar(store);

      // Updates local database according to information fetched -- Restaurants
      final AppEventsDatabase db = AppEventsDatabase();
      db.saveNewEvents(events);

      store.dispatch(SetFeupCalendar(events));

    } catch(e){
      Logger().e('Failed to get Feup Calendar: ${e.toString()}');
    }
    action.complete();
  };
}

ThunkAction<AppState> updateStateBasedOnLocalUserSigarraEvents(){
  return (Store<AppState> store) async {
    final AppSigarraEventsDatabase db = AppSigarraEventsDatabase();
    final List<CalendarItem> items = await db.events();
    store.dispatch(SetSigarraEvents(items));
  };
}

ThunkAction<AppState> getSigarraEventsFromFetcher(Completer<Null> action) {
  return (Store<AppState> store) async {
    try {
      final List<CalendarItem> events =
      await SigarraEventsFetcherHtml().getSigarraEvents(store);

      // Updates local database according to information fetched -- Restaurants
      final AppSigarraEventsDatabase db = AppSigarraEventsDatabase();
      db.saveNewEvents(events);

      store.dispatch(SetSigarraEvents(events));
    } catch (e) {
      Logger().e('Failed to get Sigarra Events: ${e.toString()}');
    }
    action.complete();
  };
}